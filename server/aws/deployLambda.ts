...
const LAMBDA_FILE = path.join(LAMBDA_DIR, 'contactLambda.ts');
 ...
 */
const createZipFile = async (): Promise<string> => {
  ...
  try {
    ...
    fs.copyFileSync(LAMBDA_FILE, path.join(tempDir, 'contactLambda.ts'));
    
    // Create a package.json with only the AWS SDK dependencies
    const packageJson: PackageJson = JSON.parse(fs.readFileSync(PACKAGE_JSON, 'utf8'));
    const lambdaPackageJson: PackageJson = {
 ...
 */
const updateLambdaFunction = async (): Promise<void> => {
 ...
 */
const deployLambda = async (): Promise<void> => {
  try {
